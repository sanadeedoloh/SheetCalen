...
const App: React.FC = () => {
  ...
  // Simulator helper triggers for the server-side architecture
  const simulateServerEvent = async () => {
    try {
      triggerAlert("กำลังสั่งให้เซิร์ฟเวอร์จำลองไลฟ์สดขึ้นตารางเวลาอัตโนมัติล่วงหน้า 1 ชั่วโมง...", "success");
      const res = await fetch('/api/scheduler/simulate-event', { method: 'POST' });
      if (res.ok) {
        const data = await res.json();
        triggerAlert("เซิร์ฟเวอร์เสริมตารางสำเร็จ! เริ่มสแกนโครนและส่งสตรีมแจ้งเตือนทันที...", "success");
        fetchServerStatus();
      } else {
        triggerAlert("เซิร์ฟเวอร์ปลายทางปฏิเสธการจำลองช่วงเวลานี้", "error");
      }
    } catch (err) {
      triggerAlert("เกิดข้อผิดพลาดในการเชื่อมโยงกับ API เซิร์ฟเวอร์หลังบ้าน", "error");
    }
  };
                ...
                <div className="bg-slate-900 text-slate-100 rounded-2xl p-4 sm:p-5 border border-slate-850 shadow-xl space-y-4">
                  ...
                  <div className="space-y-2">
                    <p className="text-[10px] text-slate-350 font-bold leading-normal">
                      🎮 <span className="text-orange-400">ทดสอบจำลองกระบวนการแบบที่ 2:</span> กดปุ่มสีส้มเพื่อแทรกเหตุการณ์จริงในอีก 59 นาทีข้างหน้า เซิร์ฟเวอร์จะตรวจหาคู่สัญญาและดีสแพชข้อความแจ้งเตือนมายังมือถือคุณทันทีโดยอัตโนมัติ!
                    </p>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1">
                      <button
                        ...
                        title="สั่งเซิร์ฟเวอร์จำลองไลฟ์ล่วงหน้า 1 ชั่วโมงเพื่อส่งแจ้งเตือนระบบแบบที่ 2"
                      >
                        ⏰ จำลองไลฟ์ล่วงหน้า 1 ชม.
                      </button>
                      <button
