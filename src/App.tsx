...
const App: React.FC = () => {
  ...
  const [notifiedOneHourReminders, setNotifiedOneHourReminders] = useState<string[]>([]);
  const [lastServerAlertId, setLastServerAlertId] = useState<string>(() => {
    return localStorage.getItem('last_server_alert_id') || '';
  });
  const [serverActivityLogs, setServerActivityLogs] = useState<{ id: string; message: string; timestamp: string }[]>([]);
  ...
  // ตรวจจับและทำการแจ้งเตือนล่วงหน้า 1 ชั่วโมง สำหรับ Tech ที่ผู้ใช้งานติดตาม
  useEffect(() => {
    if (!notificationEnabled || subscribedTechs.length === 0 || events.length === 0) return;

    const checkOneHourReminders = () => {
      const now = new Date();
      
      events.forEach(event => {
        // กรองหา Tech ที่ตรงกับสิทธิ์ที่สมัครไว้ของเครื่องนี้
        const matchedTechs = event.techs.filter(t => subscribedTechs.includes(t));
        if (matchedTechs.length === 0) return;

        // ตรวจสอบความถูกต้องของวันจัดไลฟ์ (เป็นวันนี้หรือไม่)
        ...
        if (!isSameDay(eventDate, now)) return;

        // แยกชั่วโมงและนาทีเวลาเริ่มต้นของคิวงาน (เช่น "12:00")
        if (!event.time || !event.time.includes(':')) return;
        const [hours, minutes] = event.time.split(':').map(Number);
        if (isNaN(hours) || isNaN(minutes)) return;

        // ประกอบเวลานัดพบที่ตรงกับเวลาท้องถิ่น
        const eventStart = new Date(eventDate);
        eventStart.setHours(hours, minutes, 0, 0);

        // หาผลต่างเวลาในหน่วยนาที
        const diffInMinutes = (eventStart.getTime() - now.getTime()) / 60000;

        // ถ้าไลฟ์กำลังจะเริ่มต้นภายใน 55 ถึง 65 นาทีข้างหน้า
        const isWithinWindow = diffInMinutes >= 55 && diffInMinutes <= 65;
        const reminderKey = `reminder_1hr_${event.id}_${eventStart.getTime()}`;

        if (isWithinWindow && !notifiedOneHourReminders.includes(reminderKey)) {
          setNotifiedOneHourReminders(prev => [...prev, reminderKey]);

          const title = "อย่าลืมน่ะอีก 1 ชั่วโมงมีไลฟ์";
          const body = `เวลา ${event.time} - ${event.endTime || '--:--'}\nCELEB : ${event.kol || 'Unknown KOL'}\nTECH : ${event.techs.join(', ')}`;
    ...
    // ทำงานตรวจสอบทันทีและตามด้วยการวนรอบทุกๆ 30 วินาที
    checkOneHourReminders();
    const intervalId = setInterval(checkOneHourReminders, 30000);
    return () => clearInterval(intervalId);
  }, [events, notificationEnabled, subscribedTechs, notifiedOneHourReminders]);

  // ฟังก์ชันจำลองแจ้งเตือนล่วงหน้าอีก 1 ชั่วโมงพอดิบพอดีตามผู้เขียนแจ้ง
  const triggerOneHourAdvanceDemo = () => {
    if (!('Notification' in window)) {
      triggerAlert("เบราว์เซอร์เครื่องนี้ไม่สนับสนุนการแจ้งเตือนพุช", "error");
      return;
    }
    ...
    // ไปค้นหาข้อมูลจริงจากอาร์เรย์ events ที่ดึงลงมาจาก Firebase ประจุเป็นเกรสเวิลด์
    let targetEvent: CalendarEvent | null = null;
    const now = new Date();

    // 1. ค้นหาเหตุการณ์ที่มี Tech ตรงกับที่สแตนด์บายของเครื่องนี้ และเป็นเหตุการณ์วันนี้
    if (subscribedTechs.length > 0 && events.length > 0) {
      targetEvent = events.find(event => {
        const eventDate = new Date(event.date);
        const isToday = eventDate.getFullYear() === now.getFullYear() &&
                        eventDate.getMonth() === now.getMonth() &&
                        eventDate.getDate() === now.getDate();
        const hasMatchedTech = event.techs.some(t => subscribedTechs.includes(t));
        return isToday && hasMatchedTech;
      }) || null;

      // 2. ถ้าหากไม่มีวันนี้พอดิบพอดี ให้หาเหตุการณ์ในประวัติหรืออนาคตที่มี Tech ที่ผู้ใช้ติดตาม
      if (!targetEvent) {
        targetEvent = events.find(event => event.techs.some(t => subscribedTechs.includes(t))) || null;
      }
    }

    // 3. หากยังไม่พบอีก ให้เลือกเอาเหตุการณ์ล่าสุดหรือรายการแรกสุดประคองหน้าตึกขึ้นมาทดลองสิทธิ์
    if (!targetEvent && events.length > 0) {
      targetEvent = events[0];
    }

    // ประกอบร่างข้อมูลจริงหรือใช้ตัวเดโมตัวเลือกกรณีระบบว่างเปล่า
    const timeStr = targetEvent ? `เวลา ${targetEvent.time} - ${targetEvent.endTime || '--:--'}` : "เวลา 12:00 - 14:00";
    const celebStr = targetEvent ? `CELEB : ${targetEvent.kol || 'Unknown KOL'}` : "CELEB : Preawah";
    const techsStr = targetEvent ? `TECH : ${targetEvent.techs.join(', ')}` : "TECH : Dee,Geng";

    if (targetEvent) {
      triggerAlert(`เริ่มจำลองส่งแจ้งเตือนด้วยข้อมูลจริง [KOL: ${targetEvent.kol} | TECH: ${targetEvent.techs.join(', ')}]: ข้อความจะแสดงใน 2 วินาที...`, "success");
    } else {
      triggerAlert("ระบบค้นหาประวัติตารางกิจกรรมว่างเปล่า จะใช้คลาสสิกโมเดลสำรองเพื่อส่งแสดงผลตัวอย่างใน 2 วินาที...", "success");
    }

    setTimeout(() => {
      showSystemNotification(
        "อย่าลืมน่ะอีก 1 ชั่วโมงมีไลฟ์",
        `${timeStr}\n${celebStr}\n${techsStr}`
      );
    }, 2000);
  };
  ...
  // แปลง VAPID Public Key จาก format Base64 สำหรับเบราว์เซอร์
  ...
  // บันทึกการตั้งค่าการแจ้งเตือนของเครื่องลงบนคลาวด์ Firebase และติดตั้งระบบ Real Web Push (VAPID)
  const saveSubscriptionSettings = async (enabled: boolean, techs: string[], name: string) => {
    try {
      let webPushSubscriptionJson: any = null;

      if (enabled) {
        // ขอติดตั้งสิทธิ์ Web Push แท้จริง
        if ('serviceWorker' in navigator && 'PushManager' in window) {
          try {
            const reg = await navigator.serviceWorker.ready;
            
            // ดึง Public Key จากฝั่งหลังบ้าน
            ...
            if (keyData?.success && keyData?.publicKey) {
              ...
              // ตรวจสอบและลงทะเบียน Push Subscription ของ OS
              let sub = await reg.pushManager.getSubscription();
              
              // If already subscribed under a different key or something, clear it or renew
              if (sub) {
                try {
                  // Keep or re-use existing
                  webPushSubscriptionJson = sub.toJSON();
                } catch (jsonErr) {
            ...
            console.warn("ไม่สามารถผูกติดตั้ง Push Subscription ได้ (อาจเกิดจากการปิดกั้นสิทธิ์หรือรันบน Preview Iframe):", pushErr);
          }
        }
      } else {
        // ปิดแจ้งเตือน ยินยอมให้เพิกถอนสิทธิ์ Push ในเครื่องย้อนหลัง
        if ('serviceWorker' in navigator && 'PushManager' in window) {
          try {
            const reg = await navigator.serviceWorker.ready;
            ...
            console.warn("Unsubscribe push error:", unsubErr);
          }
        }
      }

      const subscriptionDocData: any = {
        deviceId,
      ...
      if (webPushSubscriptionJson) {
        subscriptionDocData.webPushSubscription = webPushSubscriptionJson;
      } else if (!enabled) {
        subscriptionDocData.webPushSubscription = null;
      }

      await setDoc(doc(db, 'artifacts', appId, 'subscriptions', deviceId), subscriptionDocData, { merge: true });

      setNotificationEnabled(enabled);
      setSubscribedTechs(techs);
      setDeviceName(name);
      
      if (enabled && webPushSubscriptionJson) {
        triggerAlert("บันทึกข้อมูลเรียบร้อย! ระบบได้เชื่อมต่อ Web Push กับระบบปฏิบัติการเครื่องของคุณเรียบร้อย (แจ้งเตือนได้แม้ปิดหน้าเว็บ) 🎉", "success");
      } else if (enabled) {
        triggerAlert("บันทึกการตั้งค่าติดตามแล้ว! (เปิดรับแจ้งเตือนเมื่อเปิดแท็บหน้าแรกทิ้งไว้)", "success");
      } else {
        triggerAlert("ยกเลิกการส่งสัญญาณแจ้งเตือนไปยังอุปกรณ์นี้สำเร็จ", "success");
      }
    } catch (err) {
  ...
  // --- Real-time Alert Sync from Server-Side Scheduler (แบบที่ 2) ---
  useEffect(() => {
    if (!notificationEnabled || !deviceId) return;

    console.log("Listening to Server-Side pushed Alerts from Firestore (แบบที่ 2)...");
    const docRef = doc(db, 'artifacts', appId, 'subscriptions', deviceId, 'alerts', 'latest');
    
    const unsubscribeAlerts = onSnapshot(docRef, (docSnap) => {
      if (docSnap.exists()) {
        const alertData = docSnap.data();
        if (alertData && alertData.id && alertData.id !== lastServerAlertId) {
          // If the alert ID is new, trigger local notification immediately!
          setLastServerAlertId(alertData.id);
          localStorage.setItem('last_server_alert_id', alertData.id);

          // Force browser push notification banner!
          showSystemNotification(alertData.title, alertData.body);
          
          triggerAlert(`📍 เซิร์ฟเวอร์ส่งแจ้งเตือน: ${alertData.eventKol || 'KOL'} (แบบที่ 2)`, "success");
        }
      }
    }, (err) => {
      console.warn("Error listening to server alerts stream:", err);
    });

    return () => unsubscribeAlerts();
  }, [notificationEnabled, deviceId, lastServerAlertId]);

  // --- Real-time Server Activity Log Subscription ---
  useEffect(() => {
    console.log("Subscribed to Server Chron logs...");
    const schedulerStateDocRef = doc(db, 'artifacts', appId, 'scheduler', 'state');
    
    const unsubscribeSchedulerLogs = onSnapshot(schedulerStateDocRef, (docSnap) => {
      if (docSnap.exists()) {
        const data = docSnap.data();
        if (data && data.activityLogs) {
          setServerActivityLogs(data.activityLogs);
        }
      }
    }, (err) => {
      console.warn("Scheduler logs subscription offline:", err);
    });

    return () => unsubscribeSchedulerLogs();
  }, []);
  ...
  const fetchServerStatus = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setServerStatusInfo(data);
      }
    } catch (e) {
      console.warn("Server status API skipped or offline in dev mode:", e);
    }
  };

  useEffect(() => {
    fetchServerStatus();
    const interval = setInterval(fetchServerStatus, 15050);
    return () => clearInterval(interval);
  }, []);

  // Simulator helper triggers for the server-side architecture (สแกนหาตารางงานจัดไลฟ์ที่ใกล้ที่สุด พร้อมพุชแจ้งเวลานับถอยหลัง)
  const simulateServerEvent = async () => {
    try {
      triggerAlert("กำลังขอให้เซิร์ฟเวอร์หลังบ้านประเมินตาราง เพื่อประมวลผลคิวงานจัดไลฟ์ที่ใกล้ที่สุด...", "success");
      const res = await fetch('/api/scheduler/simulate-event', { method: 'POST' });
      if (res.ok) {
        const data = await res.json();
        if (data.isMocked) {
          triggerAlert(`💡 [ไม่มีนัดล่วงหน้าในระบบ] เซิร์ฟเวอร์จึงจำลองงานให้ถัดไป: [KOL: ${data.nearestEvent.kol}] เวลา ${data.nearestEvent.time} (นับถอยหลังอีกประมาณ ${data.countdown.formatted})`, "success");
        } else {
          triggerAlert(`🎯 บรรลุผลสำเร็จ! ตรวจพบงานจริงใกล้ที่สุด: [KOL: ${data.nearestEvent.kol}] รอบเวลา ${data.nearestEvent.time} (นับถอยหลังคงเหลืออีก ${data.countdown.formatted} จะเริ่มทำการพุชเตือนตรงเข้ามือถือคุณ)`, "success");
        }
        fetchServerStatus();
      } else {
        triggerAlert("เซิร์ฟเวอร์หลังบ้านส่วนการคำนวณคิวรายงานข้อผิดพลาด", "error");
      }
    } catch (err) {
      triggerAlert("เกิดข้อผิดพลาดในการเชื่อมโยงและสตรีมคำนวณจากควิกเซิร์ฟเวอร์", "error");
    }
  };

  const forceServerScan = async () => {
    try {
      triggerAlert("กำลังส่งสัญญาณให้ระบบเซิร์ฟเวอร์สแกนตารางล่วงหน้า (Cron Scan)...", "success");
      const res = await fetch('/api/scheduler/simulate-run', { method: 'POST' });
      if (res.ok) {
        triggerAlert("เซิร์ฟเวอร์รายงาน: สแกนและตรวจคู่สัญญาเรียบร้อยแล้ว", "success");
        fetchServerStatus();
      } else {
        triggerAlert("บริการวิเคราะห์เซิร์ฟเวอร์ยังไม่เปิดการสแกนแบบบังคับ", "error");
      }
    } catch (err) {
      triggerAlert("การบังคับสแกนตารางเซิร์ฟเวอร์ล้มเหลว", "error");
    }
  };
            ...
            >
              ...
              <div className="p-5 sm:p-6 bg-slate-50 overflow-y-auto max-h-[60vh] sm:max-h-[65vh] space-y-5 scrollbar-thin">
                ...
                <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-100 shadow-sm space-y-4">
                 ...
                 {/* 4. ระบบเซิร์ฟเวอร์แจ้งเตือนอัตโนมัติ (แบบที่ 2: Server-Side Push Scheduler) */}
                <div className="bg-slate-900 text-slate-100 rounded-2xl p-4 sm:p-5 border border-slate-850 shadow-xl space-y-4">
                  <div className="flex items-center justify-between gap-2 border-b border-slate-800 pb-2.5">
                    <div className="space-y-0.5">
                      <h4 className="text-xs sm:text-sm font-black text-orange-400 uppercase tracking-tight flex items-center gap-1.5 font-bold">
                        🖥️ สถานะเซิร์ฟเวอร์ภูมิหลัง (แบบที่ 2: Server-Side Push)
                      </h4>
                      <p className="text-[10px] text-slate-400 font-medium">ควบคุมระบบรันไทม์ NodeJS Cron Scheduler บน Cloud Run คอยเฝ้าจับเวลาส่งพุชแจ้งเตือนอัตโนมัติ</p>
                    </div>
                    <span className="text-[8px] font-black bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 px-2 py-0.5 rounded uppercase tracking-wider shrink-0 animate-pulse">
                      ONLINE
                    </span>
                  </div>

                  {/* แผงแสดงสืบค้น Diagnostics */}
                  <div className="grid grid-cols-2 gap-2.5">
                    <div className="p-2.5 bg-slate-950 rounded-xl border border-slate-800/60 text-center">
                      <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Active Subscriptions</p>
                      <h5 className="text-lg font-black text-orange-400 mt-1">
                        {serverStatusInfo?.activeSubscriptionsCount ?? "กำลังโหลด..."}
                      </h5>
                    </div>
                    <div className="p-2.5 bg-slate-950 rounded-xl border border-slate-800/60 text-center">
                      <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Scheduler Mode</p>
                      <h5 className="text-[10px] font-black text-slate-200 mt-2 uppercase truncate select-none">
                        PROD ACTIVE
                      </h5>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <p className="text-[10px] text-slate-350 font-bold leading-normal">
                      🎮 <span className="text-orange-400">ทดสอบจำลองระบบแบบที่ 2:</span> ค้นหาจัดคู่ตารางเวลาอัตโนมัติ คำนวณช่วงเวลาการเริ่มล่วงหน้าของคิวงานที่ใกล้เคียงที่สุดบน Firebase และส่งสัญญาณเตือนคุณอย่างรวดเร็วพร้อมเลขอายุนับถอยหลัง!
                    </p>

                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-1">
                      <button
                        onClick={simulateServerEvent}
                        className="py-2 px-1 bg-orange-600 hover:bg-orange-550 text-white rounded-xl text-[9px] font-black tracking-wider uppercase transition-all shadow cursor-pointer text-center"
                        title="สั่งเซิร์ฟเวอร์คนงานสแกนหาไลฟ์ที่ใกล้ที่สุดพร้อมคำนวณหักลบเวลานับถอยหลัง"
                      >
                        ⏰ ตรวจตารางใกล้สุด (Scan Nearest)
                      </button>
                      <button
                        onClick={forceServerScan}
                        className="py-2 px-1 bg-slate-800 hover:bg-slate-750 text-slate-200 rounded-xl text-[9px] font-black tracking-wider uppercase transition-all border border-slate-750 shadow cursor-pointer text-center"
                        title="บังคับสแกนตารางแบบ Cron ทันที"
                      >
                        🔄 สแกนตาราง (Force Cron)
                      </button>
                      <button
                        onClick={triggerDemoNotification}
                        className="py-2 px-1 bg-slate-950 hover:bg-slate-900 text-slate-400 rounded-xl text-[9px] font-black tracking-wider uppercase transition-all border border-slate-800 shadow cursor-pointer text-center"
                        title="จำลองแจ้งเตือนด่วนผ่านบราวเซอร์ตรงฝั่งลูกข่าย"
                      >
                        ⚡ อัปเดตควิกเดโม (Client)
                      </button>
                    </div>
                  </div>
                </div>

                {/* 5. เซิร์ฟเวอร์ล็อกสด และประวัติการแจ้งเตือน */}
                <div className="bg-white rounded-2xl p-4 sm:p-5 border border-slate-100 shadow-sm space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* บล็อก 5.1: SERVER CRON LOGS */}
                    <div className="space-y-2.5">
                      <h4 className="text-[11px] font-black text-slate-900 uppercase flex items-center gap-1.5 font-bold tracking-tight">
                        <Database size={13} className="text-slate-500" /> 📡 ล็อกเรียลไทม์เซิร์ฟเวอร์หลังบ้าน (Server Cron logs)
                      </h4>
                      <div className="p-3 bg-slate-950 rounded-xl border border-slate-900 text-[9px] font-mono text-emerald-400 h-44 overflow-y-auto space-y-1.5 scrollbar-thin shadow-inner pr-1">
                        {serverActivityLogs.length === 0 ? (
                          <p className="text-slate-500 italic text-center py-10 uppercase">Waiting for backend cron records...</p>
                        ) : (
                          serverActivityLogs.map(log => (
                            <div key={log.id} className="border-b border-slate-900 pb-1 last:border-0">
                              <span className="text-slate-500 font-bold select-none">[{new Date(log.timestamp).toLocaleTimeString('th-TH')}]</span>{" "}
                              <span className="text-slate-250 font-medium whitespace-pre-wrap">{log.message}</span>
                            </div>
                          ))
                        )}
                      </div>
                    ...
                    {/* บล็อก 5.2: LOCAL LOGS */}
                    <div className="space-y-2.5">
                      <h4 className="text-[11px] font-black text-slate-900 uppercase flex items-center gap-1.5 font-bold tracking-tight">
                        <History size={13} className="text-slate-500" /> 📱 ประวัติการขึ้นสติกแจ้งเตือนเครื่องนี้ (Log History)
                      </h4>
                      <div className="p-3 bg-slate-50 rounded-xl border border-slate-150 text-[9px] h-44 overflow-y-auto space-y-2 pr-1 scrollbar-thin">
                        {notificationLog.length === 0 ? (
                          <p className="text-slate-400 italic text-center py-10 uppercase font-black">ยังไม่พบบันทึกการรับ Push เกตเวย์</p>
                        ) : (
                          notificationLog.map(log => (
                            <div key={log.id} className="p-2 bg-white border border-slate-100 rounded-lg space-y-1">
                              <div className="flex justify-between items-center gap-1.5">
                                <span className="font-extrabold text-orange-650 uppercase tracking-wider">{log.title}</span>
                                <span className="text-[8px] font-bold text-slate-400 font-mono shrink-0">{log.time}</span>
                              </div>
                              <p className="text-slate-600 font-medium leading-normal whitespace-pre-wrap">{log.body}</p>
                            </div>
                          ))
                        )}
                      </div>
