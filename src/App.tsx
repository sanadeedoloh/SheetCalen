...
const App: React.FC = () => {
  ...
  // จัดการยืนยันตัวตนอัตโนมัติเมื่อเปิดหน้าเว็บขึ้นมาครั้งแรกสุด
  useEffect(() => {
    const initAuth = async () => {
      try {
        await signInAnonymously(auth);
      } catch (err) {
        console.warn("Firebase auth failed, falling back to local storage and JSONP mode:", err);
        setStatusMsg("โหมด Local: ดำเนินการต่อด้วยการจัดเก็บข้อมูลภายในบราวเซอร์");
        triggerAlert("เข้าสิทธิ์คลาวด์ไม่สำเร็จ เพื่อให้ระบบทำต่อได้จึงสลับไปยังโหมดจัดเก็บข้อมูลในบราวเซอร์", "error");
        setUser({ uid: 'local-temp-user', isAnonymous: true } as any);
      }
    };
    initAuth();
  ...
  // ดึงข้อมูลแคชถาวรจาก Firebase และลิงก์อัปเดตล่าสุดมาติดตั้งเมื่อโหลดแอปเสร็จ
  useEffect(() => {
    if (!user) return;
    ...
    const fetchStoredData = async () => {
      ...
      if (user.uid === 'local-temp-user') {
        setStatusMsg("ดาวน์โหลดข้อมูลสำเร็จผ่านโหมดประมวลผลภายในขอบเขตบราวเซอร์ (Local Only)");
        // สั่งดึงข้อมูลทันที
        try {
          const savedUrl = localStorage.getItem('shopee_live_ops_apps_script_url');
          if (savedUrl) {
          ...
          console.warn("Local storage url read failed:", err);
        }
      ...
      setStatusMsg("กำลังดาวน์โหลดข้อมูลสำรองถาวรจาก Firebase...");
      
      const configPath = `artifacts/${appId}/public/data/settings/config`;
      const statePath = `artifacts/${appId}/public/data/live_calendar/state`;

      try {
        // ดึงการตั้งค่า URL จาก Firebase
      ...
      try {
        // ดึงชุดข้อมูลปฏิทินจาก Firebase
        ...
        if (eventsDoc.exists()) {
          ...
          setEvents(parsed);
          setIsAuthorized(true);
          setStatusMsg(`โหลดข้อมูลจากคลาวด์ Firebase เรียบร้อย (${parsed.length} กิจกรรม)`);
          triggerAlert(`โหลดข้อมูลจากคลาวด์สำเร็จ (${parsed.length} คิวงาน)`, "success");
        } else {
          setStatusMsg("ยังไม่มีข้อมูลใน Firebase คลาวด์ เริ่มดาวน์โหลดจากชีต หรือ Seed ข้อมูลทดสอบ...");
          fetchDataJSONP();
        }
      } catch (err) {
        handleFirestoreError(err, OperationType.GET, statePath);
        fetchDataJSONP(); // ดึงข้อมูลจากชีตเป็นแผนสำรองเพื่อป้องกันจอดำ
      } finally {
        setLoading(false);
      }
    };
  ...
  // ฟังก์ชันบันทึกข้อมูลทับถาวรลงใน Firebase Firestore
  const saveToFirebase = async (dataToSave: CalendarEvent[]) => {
    if (!auth.currentUser || auth.currentUser.uid === 'local-temp-user' || user?.uid === 'local-temp-user') {
      setStatusMsg(`เซฟข้อมูลลงเบราเซอร์สำเร็จ (${dataToSave.length} รายการ)`);
      return;
    }
    setStatusMsg("กำลังจัดเก็บชุดข้อมูลลงฐานข้อมูล Firebase...");
    const statePath = `artifacts/${appId}/public/data/live_calendar/state`;
    ...
    try {
      ...
      setStatusMsg(`ซิงก์ข้อมูลลง Firebase สำเร็จ (${dataToSave.length} รายการ)`);
      triggerAlert(`ซิงก์ข้อมูลลง Firebase คลาวด์ สำเร็จ (${dataToSave.length} รายการ)`, "success");
    } catch (error) {
      handleFirestoreError(error, OperationType.WRITE, statePath);
      setStatusMsg("บันทึกข้อมูลลง Firebase ไม่สำเร็จ");
      triggerAlert("ซิงก์ข้อมูลลง Firebase คลาวด์ ไม่สำเร็จ", "error");
    }
  };
  ...
  const handleUpdateUrlSubmit = async (e: React.FormEvent) => {
    ...
    if (tempUrl.trim() !== '') {
      ...
      try {
        ...
        if (auth.currentUser?.uid !== 'local-temp-user') {
